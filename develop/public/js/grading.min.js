$(document).ready(function() {
    ...
    }, setSessionInfo = function()
    {
        ajax('GET', '/api/judges/sessions/' + cid + '/rooms/' + rid + '/status', {}, function (a) {
            if(a.success){
                $('.info-title').empty();
                $('.info-title').append('<h4 class="mt-0 mb-1" style="text-indent: 1em;">' + translate(a.data.competition.title).split('$?')[0] + '</h4>');
                if (a.data.competition.title.split('$?')[1])$('.info-title').append('<h4 class="mt-0 mb-1" style="text-indent: 1em;">' + translate(a.data.competition.title).split('$?')[1] + '</h4>');
    ...
    }, saveArticle = function(type, id)
    {
        var data = {
            ...
            'score_5': $('.' + type + 'score[data-id="5"]').val(),
        };
