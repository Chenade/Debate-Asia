...
function getWordCount()
{
    ...
    const words = (characters > 0) ? $('#input').val().trim().split(/\s+/).length : 0;
    $('#words-count').text(words);
}

function beforeStart(start_time)
{
    const _time = moment().utc(false);
    start = moment.unix(start_time).utc(false);
    ...
    else
    {
        ...
        $('#CountDownTime').text(`${m}:${s}`);
        setTimeout(() => { beforeStart(start_time); }, 1000);
    }
}
...
function updateUI(stage, data) {
	...
	var config = handleSessionStatus(data.session.date, data.session, stage);
    switch (config.status) {
        case 0:
            beforeStart(data.session.date);
            $('#tag').text(data.session.group_name);
            $('#stage0').css('display', 'inline');
            $('.countDown').css('display', 'inline');
            $('.roles').css('display', 'none');
            $('#videos').attr('src', 'https://player.youku.com/embed/XNjIyNjUwODkwMA==');
            $('#moveStage').data('stage', 1);
            break;
        case 1:
            getRemainingTime(data.session.date, data.session, config.status);
            ...
            $('#character-count').text($('#input').val().length);
        ...
        case 2:
            getRemainingTime(data.session.date, data.session, config.status);
            displaySessionInfo(data);
            // console.log(data.my_argument);
            $('#curStage').text(langDict['buffer']);
            $('#stage1').css('display', 'inline');
            $('#stage0').css('display', 'none');
            $('#input').val(trimquotes(data.articles.content));
            $('#input').data('aid', data.articles.article_id);
...
function displaySessionInfo(data) {
    $('#tag').text(data.session.group_name);
    // $('#title').empty();
    $('#pos_title').text(data.session.pos_title);
    $('#neg_title').text(data.session.neg_title);
...
function handleSessionStatus(start_time, data, stage) {
    const _time = moment().utc(false);
    var start = null;
    switch (stage) {
        case 1:
            ...
            break;
        case 2:
            start = moment.unix(start_time).utc(false).add(data.t_write, 'minutes');
            minutes = data.t_read;
            break;
        case 3:
            start = moment.unix(start_time).utc(false).add(data.t_write + 5, 'minutes');
            minutes = data.t_read;
            break;
        case 4:
            start = moment.unix(start_time).utc(false).add(data.t_write + 5 + data.t_read, 'minutes');
...
function getRemainingTime(start_time, data, stage) {
    const _time = moment().utc(false);
    // var start = null;
    var minutes = 0;
    switch (stage) {
        case 1:
            ...
            break;
        case 2:
            start = moment.unix(start_time).utc(false).add(5, 'minutes');
            minutes = data.t_write;
            break;
        case 3:
            start = moment.unix(start_time).utc(false).add(data.t_write + 5, 'minutes');
    ...
    else
    {
        ...
        countDown = setTimeout(() => { getRemainingTime(start_time, data, stage); }, 1000);
    }
}
...
function initSessionInfo(a)
{
    ...
    {
        if (a.data.rounds[i].role > 2) continue;
