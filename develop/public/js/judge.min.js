            ...
            for (const i in a.data)
            {
                ...
                for (const j in a.data[i].groups)
                {
                    for (const k in a.data[i].groups[j].sessions)
                    {
                        ...
                        {
                            if (a.data[i].groups[j].sessions[k].rounds[l].status >= 2)
                            {
                                $('#graded_lst').append(`
                                    <div class="col-11 bg-dark" style="min-height: 80px; margin: 10px; padding: 1em; color: #FFFFFF;" data-id="${k}">
                                        ...
                                        <div class="d-flex justify-content-between flex-wrap">
                                            <div class="col-12 col-md-8">
                                                <h5 class="mb-0 mt-0" style="color: #FFFFFF;">${a.data[i].groups[j].sessions[k].session_name}｜${langDict['sessionNo']}：${l}</h5>
                                                <p class="pl-3 mb-0 mt-0" style="color: #FFFFFF;">${langDict['pos']}${langDict['title']}：${a.data[i].groups[j].sessions[k].rounds[l].pos_title}</p>
                                                <p class="pl-3 mb-0 mt-0" style="color: #FFFFFF;">${langDict['neg']}${langDict['title']}：${a.data[i].groups[j].sessions[k].rounds[l].neg_title}</p>
                                            </div>
                                            <div class="col-12 col-md-4 d-flex align-items-center">
                                                <button class="btn btn-light mb-0 start-btn" data-session_id="${k}" data-room_number=${l}>${langDict['view']}</button>
                                            </div>
                                        </div>
                                    </div>
                                `);
                                ...
                                $('#ungraded_lst').append(`
                                    <div class="col-11 bg-dark" style="min-height: 80px; margin: 10px; padding: 1em; color: #FFFFFF;" data-id="${k}">
                                        ...
                                        <div class="d-flex justify-content-between flex-wrap">
                                            ...
                                            <div class="col-12 col-md-4 d-flex align-items-center">
                                                <button class="btn btn-light mb-0 start-btn" data-session_id="${k}" data-room_number=${l}>${langDict['start']}</button>
                                            </div>
                                        </div>
                                    </div>
                                `);
