$(document).ready(function() {
    ...
    ajax('GET', '/api/judges/list', {}, function (a) {
        if(a.success){
            ...
            $('#name').html(a.data.usr.name_cn);
            $('#account').html(a.data.usr.account);
            $('#school_name').html(a.data.usr.school_cn);
            var box = "";
            $('#pastEvent').empty();
            for (const i in a.data.competition)
            {
                ...
                box += '<h5 class="mb-0 mt-0" style="color: #FFFFFF;">' + _this.info.tag + '</h5>';
                box += '<hr>';
                box += '<div class="d-flex flex-wrap">';
                    box += '<div class="col-12 col-sm-8">';
                        box += '<h5 class="mb-1 mt-0" style="color: #FFFFFF;">' + langDict['sessionTime'] + '：' + moment.unix(_this.info.date).format("YYYY 年 MM 月 DD 日 HH:mm") + '</h5>';
                        box += '<h5 class="mb-1 mt-0" style="color: #FFFFFF;">' + langDict['title'] + '：' + _this.info.title.split('$?')[0] + '</h5>';
                        if(_this.info.title.split('$?')[1]) box += '<h5 class="mb-1 mt-0" style="color: #FFFFFF; text-indent: 3em;">' + _this.info.title.split('$?')[1] + '</h5>';
                    box += '</div>';
                    if ( _this.status == 1)
                    {
                        box += '<div class="col-12 col-sm-4">';
                                box += '<div class="d-flex justify-content-center align-items-end"><button class="btn btn-light competition-btn" data-cid="' + _this.info.id + '">' + langDict['view'] + langDict['round'] + '</button><div>';
                        box += '</div>';
                    }
                box += '</div>';
                box += '</div>';
    ...
    $(document).on('click', '.start-btn', function () {
        ...
        const cid = $(this).data('cid');
        location.href = "/judge/" + cid + "/session/" + rid;
    });
