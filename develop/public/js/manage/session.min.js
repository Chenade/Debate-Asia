...
var download_file = function(text, filename)
{
    ...
    printWindow.document.write('<div style="width: 100%; text-align: center; font-size: 11px; line-height: 2em;">');
...
$(document).ready(function(){
    ...
    function generateDownload(_this)
    {
        let text = "";
        let sub_title = '';
        sub_title += _this.session_name + '<br />';
        sub_title += `<p style="font-size:12px; font-weight: bold; margin: 0;">${_this.title}</p>`; 
        sub_title += _this.name_cn + '　' + _this.school_cn + '<br />';
        for(const j in _this.article)
        {
            text += '<div style="margin-bottom: 2em;">';
            text += sub_title;
            text += _this.role  + ((j == 0) ? '申論' : '駁論') + '：<br /><br />';
            text += _this.article[j] + '<br />';
            text += '</div>';
        }
        text += '<hr>';
        return text;
    ...
    $('#download-articles-btn').click(function () {
        ...
        {
            text += generateDownload(object[i]);
        }
        download_file(text, object.pos.session_name + '辯論稿');
    });
