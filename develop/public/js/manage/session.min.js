...
function getSessionInfo() {
    ...
    ajax('GET', '/api/sessions/' + sessionId + '/rounds', {}, function (a) {
        if(a.success){
            var rows = "";
            $('#end-rounds-btn').show();
            $('#download-competition-btn').show();
            $('#download-groups-btn').show();
            ...
            autoRefresh = setTimeout(function () { getSessionInfo();}, 60000);
        }else
...
$(document).ready(function(){
    ...
    $('#download-articles-btn').click(function () {
        ...
        for (const i in content) {
            ...
            text += content[i].title + '<br />' + content[i].subtitle + '<br />' ;
                text += '<div style="width: 100%; text-align: left; font-size: 12px; line-height: 2em;">';
                text += content[i].text;
                text += '</div>';
            text += '</div>';
        }
    ...
    function generatePatchDownload(session, pos, neg) {
        let sections = [];

        for (const i in pos) {
            let sub_title = `
                ${session.session_name}<br />
                    <p style="font-size:14px; font-weight: bold; margin: 0;">${session.pos_title}</p>
                    <p style="font-size:14px; margin: 0;">${pos[0].name_cn} ${pos[0].school_cn}</p>`;
            sections.push({
                title: sub_title,
                subtitle: `正方 ${i == 0 ? '申論' : '駁論'}：<br /><br />`,
                text: pos[i].content ?? '' + '<br />',
            });
            sub_title = `
                ${session.session_name}<br />
                    <p style="font-size:14px; font-weight: bold; margin: 0;">${session.neg_title}</p>
                    <p style="font-size:14px; margin: 0;">${neg[0].name_cn} ${neg[0].school_cn}</p>`;
            sections.push({
                title: sub_title,
                subtitle: `反方 ${i == 0 ? '申論' : '駁論'}：<br /><br />`,
                text: neg[i].content ?? ''+ '<br />',
            });
        }
    ...
    function parsePatchDownload(data)
    {
        let text = '';
        for (const i in data)
        {
            const _session = data[i];
            for (const j in _session.rounds)
            {
                const _round = _session.rounds[j];
                if (Object.keys(_round).length == 2)
                {
                    let content = (generatePatchDownload(_session, _round[1], _round[2]));
                    for (const i in content) {
                        text += '<div style="page-break-after: always;">';
                        text += content[i].title + '<br />' + content[i].subtitle + '<br />' ;
                            text += '<div style="width: 100%; text-align: left; font-size: 12px; line-height: 2em;">';
    ...
    $('#download-competition-btn').click(function () {
        ajax('GET', '/api/articles/competition/' + $('#select_competition').val() + '', {}, function (a) {
            if (a.success)
            {
                const text = parsePatchDownload(a.data);
                if (text.length > 0)
                    download_file(text, 'competition');
            }
        });
    });

    $('#download-groups-btn').click(function () {
        ajax('GET', '/api/articles/group/' + $('#select_groups').val() + '', {}, function (a) {
            if (a.success)
            {
                const text = parsePatchDownload(a.data);
                if (text.length > 0)
                        ...
                        $(target_tab).append(`
                            <div class="d-flex flex-wrap">
                                <div class="col-12 col-lg-4" style="padding: 30px;">
                                    ...
                                    <hr>
                                    <h6>立論稿件（50分）：${_this.score_1}</h6>
                                    <h6>反駁稿件（50分）：${_this.score_2}</h6>
                                    <h6>推論證明（25分）：${_this.score_3}</h6>
                                    <h6>文字用詞（25分）：${_this.score_4}</h6>
                                </div>
