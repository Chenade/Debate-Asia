$(document).ready(function(){
    ...
    var getinfo = function(id){
        ...
        ajax('GET', '/api/competition/adminlist', {}, function (a) {
            if(a.success){
                ...
                for (const i in a.data)
                {
                    ...
                    if (_this.roomid != 0)
                    {
                        ...
                        if (_this.roomid != roomid)
                        {
                            ...
                            rows += '<td><img src="" width="50" width="50" alt="camera"/></td>';
                            rows += '<td>' + _this.status;
                            rows += '<br><span style="color: #BBB;">' + _this.updated_at + '</span></td>';
                            rows += '<td class="text-center">';
                            rows += '<button type="button" class="btn btn-success article-btn" style="margin:0" data-maid="' + _this.id + '" data-mbid="' + _next.id + '">Article</button>';
                            rows += '</td>';
                            rows += '<td>' + _next.status;
    ...
    $(document).on('click', '.article-btn', function () {
        ...
        ajax('GET', '/api/articles/adminlist/' + asid + '/' + bsid, {}, function (a) {
            if(a.success){
                document.cookie = "Authorization="+a.token;
                object.article = a.data;
                if (a.data.a[0].content) $('#a_1').val(a.data.a[0].content.substring(1, a.data.a[0].content.length-1));
                if (a.data.a[1].content) $('#a_2').val(a.data.a[1].content.substring(1, a.data.a[1].content.length-1));
                if (a.data.b[0].content) $('#b_1').val(a.data.b[0].content.substring(1, a.data.b[0].content.length-1));
                if (a.data.b[1].content) $('#b_2').val(a.data.b[1].content.substring(1, a.data.b[1].content.length-1));
