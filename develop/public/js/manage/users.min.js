...
$(document).ready(function(){
        ...
        ajax('GET', '/api/' + tid + '/'+id, {}, function (a) {
            if(a.success){
                ...
                if (a.data.locked == 1)
                {
                    ...
                    $(".lock-btn[data-action='lock']").text('已鎖定');
                    $(".lock-btn[data-action='lock']").prop('disabled', true);
                }
                else
                {
                    $(".delete-btn[data-action='delete']").show();
                    $(".delete-btn[data-action='delete']").data('id', a.data.id);
                    $(".lock-btn[data-action='lock']").data('id', a.data.id);
                    $(".lock-btn[data-action='lock']").text('鎖定');
                    $(".lock-btn[data-action='lock']").prop('disabled', false);
                }
    ...
    $('#' + tid + 'Table_filter').append(addBtn);

    $(document).on('click', '#addBtn', function () {
        $('#edit_authority').val(1).selectpicker('refresh');
        $('#edit_email').val("");
        $('#edit_account').val("");
        $('#edit_password').val("");
        $('#edit_school_cn').val("");
    ...
    $(document).on('click', '.delete-btn', function () {
        var id = $(this).data('id');
        ajax('DELETE', '/api/' + tid + '/'+id, {}, function (a) {
            if(a.success){
    ...
    $(document).on('click', '.lock-btn', function () {
        var id = $(this).data('id');
        ajax('post', '/api/' + tid + '/lock/' + id, {}, function (a) {
            if(a.success){
                $.growl.notice({message: '鎖定成功'})
                $(".delete-btn[data-action='delete']").hide();
                $(".lock-btn[data-action='lock']").text('已鎖定');
                $(".lock-btn[data-action='lock']").prop('disabled', true);
            }else
