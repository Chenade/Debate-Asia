        ...
        ajax('GET', '/api/' + tid + '/' + competitionId, {}, function (a) {
            if (!a.error)
            {
                ...
                for (const i in a.groups)
                {
                    $('#group_lst').append(`
                        <tr>
                            <td>${a.groups[i].group_name}</td>
                            <td><input type="number" class="form-control t_write" data-id="${a.groups[i].group_id}"></td>
                            <td><input type="number" class="form-control t_read" data-id="${a.groups[i].group_id}"></td>
                            <td><input type="number" class="form-control t_debate" data-id="${a.groups[i].group_id}"></td>
                            <td><button class="btn btn-success group-save-btn" data-id="${a.groups[i].group_id}"    >SAVE</button></td>
                        </tr>
                    `);
                }
    ...
    $(document).on('click', '.view-btn', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#competition_img').attr('src', '/upload/Image/proof/' + response.data.proof);
                $('.approval-btn').data('id', response.data.id);
                console.log(response);
            },
            error: function (xhr, status, error) {
                console.error(error);
