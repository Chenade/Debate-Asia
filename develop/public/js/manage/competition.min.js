    ...
    function getSessionInfo(sessionId)
    {
        ajax('GET', '/api/sessions/' + sessionId, {}, function (a) {
            ...
            for (const i in a.candidates)
            {
                ...
                {
                    let option = (a.candidates[i].session_id != null ? 'disabled' : '');
                ...
                if (a.candidates[i].session_id == a.data.id)
                {
                    $('#candidate_lst').append(`
                        <tr>
                            <td>${a.candidates[i].name_cn}</td>
                            <td>${a.candidates[i].school_cn}</td>
                            <td>
                                <input 
                                    type="number" 
                                    class="form-control rounds-room-number" 
                                    data-id="${a.candidates[i].round_id}"
                                    value="${((a.candidates[i].round_number) != null) ? a.candidates[i].round_number : ''}">
                            </td>
                            <td>
                                <select class="form-control rounds-role" data-id="${a.candidates[i].round_id}">
                                    <option value="0" ${a.candidates[i].role === null ? 'selected' : ''}>未指定</option>
                                    <option value="1" ${a.candidates[i].role === 1 ? 'selected' : ''}>正方</option>
                                    <option value="2" ${a.candidates[i].role === 2 ? 'selected' : ''}>反方</option>
                                </select>
                            </td>
                            <td>
                                <button class="btn btn-sm btn-danger rounds-remove-btn" data-id="${a.candidates[i].round_id}">REMOVE</button>
                            </td>
                        </tr>
                    `);
                }
