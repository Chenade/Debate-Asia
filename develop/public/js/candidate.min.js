$(document).ready(function() {
    ...
    ajax('GET', '/api/candidates/info', {}, function (a) {
        if (!a.error)
        {
        ...
        if(a.success){
            ...
            for (const i in a.data)
            {
                ...
                if (_this.status < 5)
                {
                    ...
                    box += '<p>' + langDict['sessionTime'] + '：' + moment.unix(_this.date).format("YYYY - MM - DD HH:mm") + '</p>';
                ...
                else
                {
                    ...
                    box += '<h5 class="mb-0 mt-0" style="color: #FFFFFF;"><span class="badge badge-warning">' + langDict['evalution'] + '</span>&emsp;' + _this.competition_name + '&emsp;-&emsp;' + _this.group_name + '</h5>';
                    box += '<hr>';
                    box += '<div class="d-flex flex-wrap">';
                        box += '<div class="col-12 col-sm-9">';
                            box += '<h5 class="mb-1 mt-0" style="color: #FFFFFF;">' + langDict['sessionTime'] + '：' + moment.unix(_this.date).format("YYYY - MM - DD HH:mm") + '</h5>';
                        ...
                        box += '<div class="col-12 col-sm-3">';
                            ...
                            if (_this.status >= 5) box += '<button class="btn btn-light download-judge-btn"" data-sid="' + _this.id +'">' + langDict['downloadjudgeFeedback'] + '</button>';
                            box += '<div>';
                        box += '</div>';
                    box += '</div>';
                    box += '</div>';
    ...
    $(document).on('click', '.download-judge-btn', function () {
        ajax('GET', '/api/rounds/' + $(this).data('sid') + '/feedback', {}, function(a){
            var text = "";
                ...
                for (const i in a.data.feedback)
                {
                    const _this = a.data.feedback[i];
                    ...
                    text += langDict['sum'] + ': ' + (_this.score_1 + _this.score_2 + _this.score_3 + _this.score_4) + ' /500\n\n';
                ...
                download_file(text, session.session_name + langDict['judgeFeedback']);
        })
    });
 });
