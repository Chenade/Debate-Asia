$(document).ready(function() {
    ...
    ajax('GET', '/api/candidates/list/' + mid + '/abc', {}, function (a) {
        if(a.success){
                ...
                if (_this.status < 4)
                {
                    ...
                    box += '<div class="d-flex flex-wrap">';
                        box += '<div class="col-12 col-sm-8">';
                            box += '<h5 class="mb-1 mt-0" style="color: #FFFFFF;">場次時間：</h5>';
                        ...
                        if ( diff < 10 && diff > -120)
                        {
                            box += '<div class="col-12 col-sm-4">';
                                    box += '<div class="d-flex justify-content-center align-items-end"><button class="btn btn-light start-btn" data-id="' + _this.roomid + '">進入比賽</button><div>';
                            box += '</div>';
                        }
                    box += '</div>';
                    box += '</div>';
                ...
                else
                {
                    ...
                    box += '<div class="d-flex flex-wrap">';
                        box += '<div class="col-12 col-sm-9">';
                            box += '<h5 class="mb-1 mt-0" style="color: #FFFFFF;">場次時間：' + moment.unix(_this.date).format("YYYY 年 MM 月 DD 日 hh:mm") + '</h5>';
                            box += '<h5 class="mb-1 mt-0" style="color: #FFFFFF;">辯題：' + _this.title + '</h5>';
                            box += '<div class="d-flex flex-wrap mt-2">';
                                box += '<div class="col-12 col-sm-6">';
                                box += '<p><span class="badge badge-secondary">正方：</span></p>'
                                box += '</div>';
                                box += '<div class="col-12 col-sm-6">';
                                box += '<p><span class="badge badge-secondary">反方：</span></p>'
                                box += '</div>';
                            box += '</div>';
                        box += '</div>';
                        box += '<div class="col-12 col-sm-3">';
                            box += '<div class="d-flex justify-content-center align-items-end flex-column">';
                            box += '<button class="btn btn-light">下載文稿</button>';
                            box += '<button class="btn btn-light">評審回饋</button>';
                            box += '<div>';
                        box += '</div>';
                    box += '</div>';
                    box += '</div>';
